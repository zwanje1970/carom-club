...
export function dashboardMobileChromeMeta(
  ...
  if (area === "client") {
    ...
    if (p === "/client/settlements") return { title: "정산 목록", backHref: "/client" };
    ...
    if (p === "/client/venues") return { title: "당구장", backHref: "/client" };
    ...
    if (/^\/client\/venues\/[^/]+\/card-publish/.test(p)) return { title: "카드 발행", backHref: "/client/venues" };
    return { title: "클라이언트", backHref: root };
  }

  if (p === "/platform") return { title: "플랫폼", backHref: null };
  if (p === "/platform/operations") return { title: "운영 관리", backHref: "/platform" };
  if (p === "/platform/site") return { title: "사이트 관리", backHref: "/platform" };
  if (p === "/platform/operations/clients" || p === "/platform/operations/clients/list") {
    return { title: "클라이언트", backHref: "/platform/operations" };
  }
  if (p.startsWith("/platform/operations/clients/")) {
    return { title: "클라이언트", backHref: "/platform/operations/clients/list" };
  }
  ...
  if (p.startsWith("/platform/site/layout")) return { title: "레이아웃", backHref: "/platform/site" };
