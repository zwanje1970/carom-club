...
export function ZoneBracketClient({ tzId, allowEdit }: { tzId: string; allowEdit: boolean }) {
  ...
  async function rerollReduction() {
    ...
    try {
      const res = await fetch(`/api/zone/tournament-zones/${tzId}/reduction-matches`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      ...
      {allowEdit && reductionRounds.length > 0 && (
          ...
          {manualMode && (
            <div className="mt-4 space-y-3">
              <p className="text-xs text-amber-700 dark:text-amber-300">
                감축경기 참가자를 직접 지정한 뒤 저장하세요. 저장 시 아래 감축경기들의 downstream 결과는 초기화됩니다.
              </p>
              ...
              <div className="space-y-2">
                {round.matches.map((m) => (
                    ...
                    >
                      ...
                      {allowEdit && (m.status === "PENDING" || m.status === "READY" || m.status === "IN_PROGRESS") && m.entryIdA && m.entryIdB && (
