...
export default function NavBarItem({ item, userName, onLogout, ...props }: Props) {
  ...
  const itemLabel = item.isCurrentUser ? (userName ?? item.label) : item.label;
  ...
  const NavBarItemComponentContents = (
    <>
      ...
      >
        {item.isCurrentUser && <UserAvatar username={userName ?? ""} className="mr-3 inline-flex h-6 w-6" />}
      ...
      {item.menu && (
        ...
        >
          <NavBarMenuList menu={item.menu} onRouteChange={props.onRouteChange} onLogout={onLogout} userName={userName} />
        </div>
      )}
    </>
  );
