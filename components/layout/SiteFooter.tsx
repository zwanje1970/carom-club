      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <div className="flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
          ...
          <div className="space-y-2 text-sm">
            ...
            {footer.footerCompanyName && (
              <p style={{ color: muted }}>{footer.footerCompanyName}</p>
            )}
            {footer.footerBusinessNumber && (
              <p style={{ color: muted }}>사업자등록번호 {footer.footerBusinessNumber}</p>
            )}
            {footer.footerCeoName && (
              <p style={{ color: muted }}>대표 {footer.footerCeoName}</p>
            )}
            {footer.footerAddress && (
              <p style={{ color: muted }}>{footer.footerAddress}</p>
            )}
            {footer.footerPhone && (
              <p style={{ color: muted }}>
                연락처{" "}
        ...
        {(footer.footerCopyright || defaultTagline) && (
          <p className="mt-6 pt-4 border-t border-white/10 text-center text-xs" style={{ color: muted }}>
            {footer.footerCopyright || defaultTagline}
          </p>
        )}
      </div>
