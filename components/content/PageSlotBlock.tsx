...
function SlotFallback({ label }: { label: string }) {
  return (
    <div className="border border-dashed border-gray-300 bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-400">
      「{label}」슬롯 — 미리보기·데이터가 없어 자리만 표시합니다.
    </div>
  );
}

function maybePlaceholder(label: string, surface: PageSlotSurface) {
  if (surface === "public") return null;
  return <SlotFallback label={label} />;
}
...
export function PageSlotBlock({
  ...
  switch (st) {
    ...
    case "tournamentIntro": {
      ...
      if (!h) return maybePlaceholder("대회 안내", surface);
      const frame = resolveHomeStructureSlotFrame(block);
      if (!frame) return maybePlaceholder("대회 안내", surface);
    ...
    case "venueIntro": {
      const h = ctx?.home;
      if (!h && surface === "public") return null;
      if (!h) return maybePlaceholder("당구장 소개", surface);
      const frame = resolveHomeStructureSlotFrame(block);
      if (!frame) return maybePlaceholder("당구장 소개", surface);
    ...
    case "venueLink": {
      const h = ctx?.home;
      if (!h && surface === "public") return null;
      if (!h) return maybePlaceholder("당구장 목록 링크", surface);
      const frame = resolveHomeStructureSlotFrame(block);
      if (!frame) return maybePlaceholder("당구장 목록 링크", surface);
    ...
    case "nanguEntry": {
      const h = ctx?.home;
      if (!h && surface === "public") return null;
      if (!h) return maybePlaceholder("난구노트·난구해결사", surface);
      const frame = resolveHomeStructureSlotFrame(block);
      if (!frame) return maybePlaceholder("난구노트·난구해결사", surface);
    ...
    case "postList": {
      ...
      if (!c) return maybePlaceholder("게시글 목록", surface);
      return (
    ...
    case "tournamentList": {
      ...
      if (!t) return maybePlaceholder("대회 목록", surface);
      return (
    ...
    case "cmsPageSections":
      return null;
    case "quickMenu":
      return maybePlaceholder("퀵 메뉴(홈 상단 메뉴는 별도 컴포넌트 — 2차 연동)", surface);
    case "homeCarousels":
      return maybePlaceholder("홈 캐러셀(미연결 슬롯 — tournamentIntro·venueIntro 슬롯 사용)", surface);
    default:
      return maybePlaceholder(String(st), surface);
  }
}
