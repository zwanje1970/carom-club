...
export default function NanguSolutionEditPage() {
    ...
    return (
      <main className="min-h-screen bg-site-bg text-site-text">
        <div className="mx-auto max-w-2xl px-4 py-6">
          ...
          <Link href={`/login?redirect=${encodeURIComponent(`/community/nangu/${postId}/solution/${solutionId}/edit`)}`} className="mt-2 inline-block text-site-primary underline">
            로그인
          </Link>
        </div>
      </main>
  ...
  if (solution.authorId !== sessionUser.id) {
    return (
      <main className="min-h-screen bg-site-bg text-site-text">
        <div className="mx-auto max-w-2xl px-4 py-6">
          <p className="text-red-600">본인 해법만 수정할 수 있습니다.</p>
          <Link href={`/community/nangu/${postId}`} className="mt-2 inline-block text-site-primary underline">
            게시글로
          </Link>
        </div>
