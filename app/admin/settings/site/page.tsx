...
export default function AdminSettingsSitePage() {
  ...
  if (loading) {
    return (
      <SectionMain>
        <SectionTitleLineWithButton icon={mdiCog} title="사이트 설정">
  ...
  return (
    <SectionMain>
      ...
      <p className="mb-6 text-sm text-gray-500 dark:text-slate-400">
        로고와 테마 색상을 설정합니다. Hero 영역(메인 상단) 문구·글자는 콘텐츠 관리 → 페이지 섹션 관리에서 메인 비주얼 배경 섹션을 수정하세요.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-site-text mb-1">탈퇴 후 재가입 가능 기간 (일)</label>
          <input
            type="number"
            min={0}
        ...
        <div>
          <p className="text-sm font-medium text-site-text mb-2">헤더 · 메뉴 색상</p>
          <p className="text-xs text-gray-500 dark:text-slate-400 mb-3">비우면 기본값(검정 배경, 회색·앰버 메뉴) 사용</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-site-text mb-1">헤더 배경</label>
              <div className="flex gap-2">
                ...
                <input
                  ...
                  className="flex-1 rounded-lg border border-site-border bg-white px-3 py-2 text-site-text font-mono text-sm"
                  placeholder="#0a0a0a"
                />
              </div>
            </div>
