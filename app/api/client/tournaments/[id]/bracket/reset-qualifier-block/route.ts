...
import { resetQualifierBlockResultsFirestore } from "../../../../../../../lib/server/firestore-tournament-brackets";
import {
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!blockId) return NextResponse.json({ error: "blockId가 필요합니다." }, { status: 400 });

  const result = await resetQualifierBlockResultsFirestore({
    tournamentId,
