...
import { substituteBracketMatchPlayer } from "../../../../../../../../../lib/server/platform-backing-store";
import {
...
export async function PATCH(
  ...
  if (mode !== "member" && mode !== "guest" && mode !== "restore") {
    return NextResponse.json({ error: "mode(member|guest|restore)이 필요합니다." }, { status: 400 });
  }

  const result = await substituteBracketMatchPlayer({
    tournamentId: id,
    matchId,
    slot,
    mode,
