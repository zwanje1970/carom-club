...
import { createMainBracketMatchesFromPlan, sortBracketPlan, syncMainBracketMatchProgressStates } from "@/lib/bracket-match-service";
...
export async function POST(
  ...
  try {
    ...
    if (type === "tournament" || gameType === "tournament") {
      ...
      // 본선 브래킷(Bracket/BracketRound/BracketMatch): 2의 거듭제곱 크기, 무작위 배치 후 BYE 패딩
      ...
      await prisma.$transaction(async (tx) => {
        await createMainBracketMatchesFromPlan(tx, {
          tournamentId,
          sortedPlan: plan,
          seedingMode: tournament.seedingMode,
          byeStrategy: tournament.byeStrategy,
        });
      ...
      void sendPushToUsers(userIds, {
        ...
        console.error("[bracket-generate] push send failed", error);
      });
      return NextResponse.json({
        ok: true,
        rounds: 1,
    ...
    const message = error instanceof Error ? error.message : "대진표 생성 중 서버 오류가 발생했습니다.";
