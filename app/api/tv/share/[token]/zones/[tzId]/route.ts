...
export async function GET(
  _request: Request,
  { params }: { params: Promise<{ token: string; tzId: string }> }
) {
  ...
  const zone = await prisma.tournamentZone.findFirst({
    where: { id: tzId, tournamentId: tournament.id },
    include: { zone: { select: { name: true, code: true } } },
  });
  if (!zone) return NextResponse.json({ error: "권역을 찾을 수 없습니다." }, { status: 404 });

  const bracket = await fetchOrImportZoneBracketSnapshotByZoneId(tournament.id, tzId);
  ...
  return NextResponse.json({
    ...
    contextLabel: [zone.name ?? zone.zone.name, zone.code ?? zone.zone.code].filter(Boolean).join(" · "),
    rounds,
    stats: {
