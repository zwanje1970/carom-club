...
- **클라이언트(hydration) 병목**: **ContentLayer(팝업)**, **홈 대회 안내 슬롯**(내 주변 찾기 등 위치 요청), **Tiptap/대진표** 등이 초기 JS와 함께 로드되어 첫 페인트·인터랙션 지연.
...
| 4 | **메인 홈 대회·당구장 블록** | 대회/당구장 카드 + 위치 재요청으로 데이터·연산 많음 | 혼합 | 서버에서 4개만 스트리밍, 클라이언트는 기존대로 위치 보강 | 서버 블로킹 제거로 체감 개선 |
