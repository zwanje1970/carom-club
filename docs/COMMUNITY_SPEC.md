...
> **난구해결 해법 편집** 롤아웃 순서(난구해결사 먼저 안정화 → 난구노트 이식): [`NANGU_SOLUTION_ROLLOUT.md`](./NANGU_SOLUTION_ROLLOUT.md)

## 목표
- 커뮤니티 메인: **허브형**
- **일반 게시판** + **난구해결사** 분리
- **난구노트**: 마이페이지 유지, 난구해결로 전송만 연결
...
| `/mypage/notes` | 난구노트 목록 (마이페이지) |
| `/mypage/notes/new` | 노트 작성 |
| `/mypage/notes/[id]` | 노트 상세/수정 |
| `/mypage/notes/[id]/edit` | 노트 편집 |

**난구노트 인증**: `/mypage/notes` 하위는 **로그인 필수**. 비회원은 서버 레이아웃에서 본문을 렌더하지 않고 로그인 유도 모달만 표시. `/api/community/billiard-notes`·`/[id]`는 비로그인 시 조회/생성/수정(및 커뮤니티 피드 목록) **401**.
...
5. 🔲 난구해결사 (등록/해법/투표/채택, 난구노트 연동)

---

## 주의
- **난구노트는 커뮤니티가 아님** (마이페이지)
- **난구해결사**는 대표 게시판 유지
- **UI 통일 금지** (게시판별 다르게)
